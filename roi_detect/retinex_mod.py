...
def exRetinexDirecao(img):        
    #variance_list=[15, 45, 5]
    #variance=100
    variance_list=[30, 160, 60]
    variance=300
    img_msr=MSR(img,variance_list)
    img=SSR(img_msr, variance)
    #img = utils_mod.medianBlurCustom(img,5,100)
    return img

def exRetinexDirecaoTone(img):        
    variance_list=[70, 5, 5]
    variance=250
    img=MSR(img,variance_list)
    img=SSR(img, variance)
    #img = utils_mod.medianBlurCustom(img,5,100)
    return img

def exRetinex(path,imgName,ext):        
    variance_list=[95, 150, 10]
    #variance_list=[1, 200, 200]
    variance=500
        
    img = cv2.imread(path+'processed/isolated_nerve/'+imgName+'.'+ext)
    img=MSR(img,variance_list)
    img=SSR(img, variance)
    #img = heatmap(img,imgName)
    utils_mod.salvarImagem(path,img,imgName,'retinex')

def exRetinexExp(img):        
    variance_list=[10, 10, 1]
    variance=300
...
def heatmap(img, imgName,ext):
    rgb_img = cv2.cvtColor(img,cv2.COLOR_BGR2RGB)
    # convert from RGB color-space to YCrCb
    ycrcb_img = cv2.cvtColor(rgb_img, cv2.COLOR_BGR2YCrCb)
    # equalize the histogram of the Y channel
    ycrcb_img[:, :, 1] = cv2.equalizeHist(ycrcb_img[:, :, 0])
    # convert back to RGB color-space from YCrCb
    image = cv2.cvtColor(ycrcb_img, cv2.COLOR_YCrCb2BGR)
    heatmap = cv2.applyColorMap(image, cv2.COLORMAP_TWILIGHT)
    return heatmap
    utils_mod.salvarImagem(heatmap,imgName,'heatmap')  


def newRetinex(path,imgName,ext,W,w,H,h):
    rSize = 300
    ...
    img_norm1 = exposure.equalize_adapthist(img, clip_limit=0.01)
    #plt.imshow(img_norm1)
    #plt.show()
    filename = "processed/retinex/" + imgName + '.' + ext
    io.imsave(os.path.join(path, filename), img_norm1)
    #utils_mod.salvarImagem(path,img_norm1,imgName,ext,'retinex')

def newRetinex1(path,imgName,ext,W,w,H,h):
    rSize = 300
    img = cv2.imread(path+imgName+'.'+ext)
    altura,largura,cores = img.shape    
    if path != 'rimone/':
    ...
    img = retinex_FM(img)
    img = SSR(img,variance)
    utils_mod.salvarImagem(path,img,imgName,ext,'retinex')

def newRetinexExp(path,imgName,img):
    rSize = 300
    #img = cv2.imread(path+imgName+'.'+ext)
    #print(h,H,w,W)
    ...
    utils_mod.salvarImagem(path,img,imgName+'_exp','retinex')
    return img
