...
def plot_confusion_matrix_fold(cm, fold, classes, normalize=False, cmap=plt.cm.Blues):
    ...
    plt.savefig(reports_dir+'/confusion_matrix/confusion_matrix_fold_'+str(fold)+'_xceptionnet.jpg')
...
# ----------------------------------------------------------------
# Criação do modelo VGG19
# ----------------------------------------------------------------
base_model = Xception(weights='imagenet', include_top=False, input_shape=(resolution, resolution, 3))
x = base_model.output
...
# ----------------------------------------------------------------
checkpoint = ModelCheckpoint(models_dir+'/checkpoints/best_xceptionnet.h5', 
...
for i, (train_index, val_index) in enumerate(skf.split(data, labels)):
    ...
    # ----------------------------------------------------------------
    model.save(models_dir+'/xceptionnet_fold_'+str(i)+'.h5')
    ...
    plt.savefig(reports_dir+'/accuracy/acc_xceptionnet_fold_'+str(i)+'.jpg')
    #plt.show()
    ...
    plt.xlabel('Epoch')
    plt.legend(['train', 'validation'])
    plt.savefig(reports_dir+'/loss/loss_xceptionnet_fold_'+str(i)+'.jpg')
    #plt.show()
    ...
    plt.plot(fpr, tpr, color='darkorange', linewidth=2, label='XceptionNet')
    ...
    plt.savefig(reports_dir+'/roc/xceptionnet_fold_'+str(i)+'.jpg')
...
print("")
# ----------------------------------------------------------------
# Função para plotar a figura com a matriz de confusão
# ----------------------------------------------------------------
def plot_confusion_matrix(cm, classes, normalize=False, cmap=plt.cm.Blues):
    ...
    plt.ylabel('True label')
    plt.xlabel('Predicted label')
    plt.savefig(reports_dir+'/confusion_matrix/confusion_matrix_xceptionnet.jpg')
...
fig, ax = plt.subplots(figsize=(12,4))
plt.plot(mean_fpr, mean_tpr, color='darkorange', linewidth=2, label='XceptionNet')
plt.plot([0, 1.05],[0, 1.05], color='navy', linestyle='--')
plt.xlim([0, 1.05])
plt.ylim([0, 1.05])
plt.xlabel('False Positive Rate')
...
plt.savefig(reports_dir+'/roc/average_roc_curve_xceptionnet.jpg')
...
models = []
for i in range(k):
    model = load_model(models_dir+'/xceptionnet_fold_'+str(i)+'.h5')
    models.append(model)
...
# ----------------------------------------------------------------
final_model = load_model(models_dir+'/xceptionnet_fold_0.h5')
final_model.set_weights(avg_weights)

# ----------------------------------------------------------------
# Salvando o modelo final
# ----------------------------------------------------------------
final_model.save(models_dir+'/final_model_xceptionnet.h5')